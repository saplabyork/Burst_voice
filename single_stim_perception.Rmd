...
per_model_bayes <- brm(response_vc ~ stim_length * ons_con * vowel + (1 + stim_length | sub),
                       data = per_data,
                       family = bernoulli(),
                       chains = 4,
                       iter = 2000,
...
table(per_data$stim_length)
table(per_data$ons_con)
table(per_data$vowel)

levels(factor(per_data$stim_length))
levels(factor(per_data$ons_con))
levels(factor(per_data$vowel))

# Check what the reference levels are
contrasts(factor(per_data$stim_length))
contrasts(factor(per_data$ons_con))
contrasts(factor(per_data$vowel))
